...
class Wpw_Fp_Users_List extends WP_List_Table {
	...
	function display_follow_post_users() {
		...
		foreach ($data as $key => $value){
			...
			$permalink = add_query_arg( array( 'page' => 'wpw-fp-post', 'postid' => $_GET['postid'], 'logid' => $value['ID'] ), admin_url( 'admin.php' ) );
		...
		switch( $column_name ) {
			...
			case 'subscribed' 	:
				...
				if( $item[ $column_name ] == '1' ) {
					$status_url = add_query_arg( array( 'page' => 'wpw-fp-post', 'postid' => $_GET['postid'], 'wpw_fp_status' => 'subscribe' ), $status_url );
					$status_link = '<a href="' . $status_url . '" >' . __( 'Yes', 'wpwfp' ) . '</a>';
				} else {
					$status_url = add_query_arg( array( 'page' => 'wpw-fp-post', 'postid' => $_GET['postid'], 'wpw_fp_status' => 'unsubscribe' ), $status_url );
    ...
    function column_user_email($item){
    	...
    	if( $item['subscribed'] == '1' ) {
    		$actions['unsubscribe'] = sprintf('<a href="?page=%s&action=%s&user[]=%s&postid=%s'.$pagestr.'">'.__('Unsubscribe', 'wpwfp').'</a>','wpw-fp-post','unsubscribe',$item['ID'],$_GET['postid'], $pagenumber );
    	} else {
    		$actions['subscribe'] = sprintf('<a href="?page=%s&action=%s&user[]=%s&postid=%s'.$pagestr.'">'.__('Subscribe', 'wpwfp').'</a>','wpw-fp-post','subscribe',$item['ID'],$_GET['postid'], $pagenumber );
    	}
    	
    	$actions['delete'] = sprintf('<a class="wpw-fp-users-delete wpw-fp-delete" href="?page=%s&action=%s&user=%s&postid=%s'.$pagestr.'">'.__('Delete', 'wpwfp').'</a>','wpw-fp-post','delete',$item['ID'],$_GET['postid'], $pagenumber );
    	...
    	if( $which == 'top' ) {
    		...
    		$html .= '<div class="alignleft actions">';
				...
				foreach ( $all_status as $key => $status ) {
					...
					$status_count = $this->model->wpw_fp_get_follow_post_users_data( $args );
    ...
    function prepare_items() {
         ...
         */
		$data = $this->display_follow_post_users();
...
$FollowedUsersListTable = new Wpw_Fp_Users_List();
...
<div class="wrap">
    <?php 
		...
		$backurl = add_query_arg( array( 'page' => 'wpw-fp-post' ), admin_url( 'admin.php' ) );
    ?>
    ...
    <form id="product-filter" method="get" class="wpw-fp-form">
        ...
        <!-- Search Title -->
        <?php $FollowedUsersListTable->search_box( __( 'Search', 'wpwfp' ), 'wpwfp' ); ?>
