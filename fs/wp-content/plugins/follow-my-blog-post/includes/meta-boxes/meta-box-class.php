...
if ( ! class_exists( 'Wpw_Fp_Meta_Box' ) ) :

require_once ( WPW_FP_META_DIR . '/meta-box-validate-class.php' );
...
class Wpw_Fp_Meta_Box {
	...
	public function __construct ( $meta_box ) {
		...
		if( !defined( 'WPS_META_URL' ) ) {
			define( 'WPS_META_URL', WPW_FP_URL . 'includes/meta-boxes' ); // url to meta boxes
		} 
	...
	public function load_scripts_styles() {
		...
		if ( in_array( $typenow, $pages ) && $this->is_edit_page() ) {
			...
			wp_enqueue_style( 'wps-meta-box', $plugin_path . '/css/meta-box.css' );
      						
			// Enqueue Meta Box Scripts
			wp_enqueue_script( 'wps-meta-box', $plugin_path . '/js/meta-box.js', array( 'jquery' ), null, true );
			
			//localize script
			$newui = $wp_version >= '3.5' ? '1' : '0'; //check wp version for showing media uploader
			wp_localize_script( 'wps-meta-box','WpsMeta',array(	
																		'new_media_ui'	=>	$newui,
																		'one_file_min'	=>  __('You must have at least one file.', 'wpwfp' ))
																	);
	...
	public function check_field_select() {
		...
		wp_enqueue_style( 'wps-multiselect-select2-css', $plugin_path . '/js/select2/select2.css', array(), null );	
		wp_enqueue_script( 'wps-multiselect-select2-js', $plugin_path . '/js/select2/select2.js', array( 'jquery' ), false, true );    
	}
	...
	public function insert_images() {
		...
		if ( ! isset( $_POST['wps-meta-insert'] ) || empty( $_POST['attachments'] ) )
		return;
		...
		foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
			...
			$li   .= "<a title='" . __( 'Delete this image', 'wpwfp' ) . "' class='wps-meta-delete-file' href='#' rel='{$nonce}|{$post_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Delete', 'wpwfp' ) . "' /></a>";
	...
	public function wp_ajax_delete_image() {
		...
		if (strpos($field_id, '[') === false){
			check_admin_referer( "wps-meta-delete-mupload_".urldecode($field_id));
		...
		if ( $ok ){
			...
			echo json_encode( array( 'message' => __( 'Cannot delete file. Something\'s wrong.', 'wpwfp' ) ) );
			die();
		}
	}
	...
	*/
	public function check_field_date() {
		...
		if ( $this->has_field( 'date' ) && $this->is_edit_page() ) {
			// Enqueu JQuery UI, use proper version.
			$plugin_path = $this->SelfPath;
			wp_enqueue_style( 'wps-meta-jquery-ui-css', $plugin_path .'/js/jquery-ui/jquery-ui.css' );
	...
	public function check_field_time() {
		...
		if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {		  			
			...
			wp_enqueue_style( 'wps-meta-jquery-ui-css', $plugin_path .'/js/jquery-ui/jquery-ui.css' );
			wp_enqueue_script( 'jquery-ui');
			wp_enqueue_script( 'wps-meta-timepicker', $plugin_path .'/js/jquery-ui/jquery-ui-timepicker-addon.js', array( 'jquery-ui-slider','jquery-ui-datepicker' ), false, true );
		}    
	}
	...
	public function check_field_datetime() {
		...
		if ( $this->has_field( 'datetime' ) && $this->is_edit_page() ) {	
			...
			wp_enqueue_style( 'wps-meta-jquery-ui-css', $this->SelfPath.'/css/datetimepicker/date-time-picker.css' );
			wp_register_script('wps-datepicker-slider-script',$this->SelfPath.'/js/datetimepicker/jquery-ui-slider-Access.js');
			wp_enqueue_script('wps-datepicker-slider-script');
			wp_register_script('wps-datepicker-addon-script',$this->SelfPath.'/js/datetimepicker/jquery-date-timepicker-addon.js',array('wps-datepicker-slider-script'),false,true);
			wp_enqueue_script('wps-datepicker-addon-script');
		}    
	}
	...
	public function show() {
		...
		wp_nonce_field( WPW_FP_BASENAME, 'at_wpw_fp_meta_box_nonce' );
	...
	public function show_meta_table($checkTab = false,$tab_field='') {
		...
		echo '<table class="wps-metabox-wrapper form-table">';
	...
	public function show_field_repeater( $field, $meta ) {
		...
		echo "<div class='wps-meta-repeat".$class."' id='{$field['id']}'>";
		...
		if( count( $meta ) > 0 && is_array( $meta ) ) {
			...
			foreach( $meta as $me ) {
				...
				echo '<div class="wps-meta-repater-block">'.$mmm.'<br/><table class="repeater-table form-table" style="display: none;">';
				if( $field['inline'] ) {
					echo '<tr class="wps-meta-inline" valign="top">';
				}
				...
				echo '	</table>
						<span class="wps-meta-re-toggle"><img src="';
				...
				echo '" alt="'.__( 'Remove', 'wpwfp' ).'" title="'.__( 'Remove', 'wpwfp' ).'" id="remove-'.$field['id'].'"></div>';
				$c = $c + 1;			
			}
		}
		...
		echo '" alt="'.__( 'Add', 'wpwfp' ).'" title="'.__( 'Add', 'wpwfp' ).'" id="add-'.$field['id'].'"><br/></div>';
		
		//create all fields once more for js function and catch with object buffer
		ob_start();
		echo '<div class="wps-meta-repater-block"><table class="repeater-table form-table">';
	...
	public function show_field_textarea( $field, $meta ) {
		...
		$this->show_field_begin( $field, $meta );
		echo "<textarea class='wps-meta-textarea large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>{$meta}</textarea>";
		$this->show_field_end( $field, $meta );
	}
	...
	public function show_field_select( $field, $meta ) {
		...
		$this->show_field_begin( $field, $meta );		
		echo "<select class='wps-meta-select' name='{$field['id']}" . ( $field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'" ) . ">";
	...
	public function show_field_radio( $field, $meta ) {
		...
		foreach ( $field['options'] as $key => $value ) {
			echo "<input type='radio' class='wps-meta-radio' name='{$field['id']}' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) . " /> <span class='wps-meta-radio-label'>{$value}</span>";
		}
	...
	public function show_field_checkbox( $field, $meta ) {
		...
		$this->show_field_begin($field, $meta);
		echo "<input type='checkbox' class='rw-checkbox' value='1' name='{$field['id']}' id='{$field['id']}'" . checked(!empty($meta), true, false) . " /><p class='wps-meta'>{$field['desc']}</p></td>";
	...
	public function show_field_wysiwyg( $field, $meta,$in_repeater = false ) {
		...
		if ( version_compare( $wp_version, '3.2.1' ) < 1 || $in_repeater ) {
			echo "<textarea class='wps-meta-wysiwyg theEditor large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>{$meta}</textarea>";
		} else {
			// Use new wp_editor() since WP 3.3
			wp_editor( html_entity_decode($meta), $field['id'], array( 'editor_class' => 'wps-meta-wysiwyg', 'teeny' => true ) );
		}
	...
	public function show_field_file( $field, $meta ) {
		...
		if( ! empty( $meta ) ) {
			$nonce = wp_create_nonce( 'at_ajax_delete' );
			echo '<div style="margin-bottom: 10px"><strong>' . __( 'Uploaded files', 'wpwfp' ) . '</strong></div>';
			echo '<ol class="wps-meta-upload">';
			
				foreach( ( array )$meta[0] as $key => $att ) {
					// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
					echo "<li>" . wp_get_attachment_url( $att) . " (<a class='wps-meta-delete-file' href='#' rel='{$nonce}|$key|{$field['id']}|{$att}'>" . __( 'Delete', 'wpwfp' ) . "</a>)</li>";
				}
			echo '</ol>';
		}

		// show form upload
		echo "<div class='wps-meta-file-upload-label'>";
		echo "<strong>" . __( 'Upload new files', 'wpwfp' ) . "</strong>";
		echo "</div>";
		echo "<div class='new-files'>";
		echo "<div class='file-input'>";
		echo "<input type='file' name='{$field['id']}[]' />";
		echo "</div><!-- End .file-input -->";
		echo "<a class='wps-meta-add-file button' href='#'>" . __( 'Add more files', 'wpwfp' ) . "</a>";
	...
	public function show_field_fileadvanced( $field, $meta ) {
			...
			//echo '<ol class="wps-meta-upload">';
				...
				foreach( ( array )$meta as $key => $att ) {
					...
					if(!empty($att)) {
						...
						echo "<input type='text' name='{$field['id']}[]' value='".$att."' style='width:95%;' class='wps-upload-file-link' placeholder='http://'/>";
						echo "<span class='wps-upload-files'><a class='wps-upload-fileadvanced' href='javascript:void(0);'>".__( 'Upload a File','wpwfp')."</a></span>";
						echo "<a href='javascript:void(0);' class='wps-delete-fileadvanced'><img src='".$this->SelfPath."/images/delete-16.png' alt='".__('Delete','wpwfp')."'/></a>";
		...
		if(empty($meta[0])){
			...
			echo "<div class='file-input-advanced'>";
			echo "<input type='text' name='{$field['id']}[]' value='' style='width:95%;' class='wps-upload-file-link' placeholder='http://'/>";
			echo "<span class='wps-upload-files'><a class='wps-upload-fileadvanced' href='javascript:void(0);'>".__( 'Upload a File','wpwfp')."</a></span>";
			echo "<a href='javascript:void(0);' class='wps-delete-fileadvanced'><img src='".$this->SelfPath."/images/delete-16.png' alt='".__('Delete','wpwfp')."'/></a>";
			echo "</div><!-- End .file-input-advanced -->";
		}
		...
		echo "<a class='wps-meta-add-fileadvanced button' href='javascript:void(0);'>" . __( 'Add more files', 'wpwfp' ) . "</a>";
		//echo "</div><!-- End .new-files -->";
		echo "</td>";
		$this->show_field_end( $field, $meta );
	}
	...
	public function show_field_image( $field, $meta ) {
		...
		$this->show_field_begin( $field, $meta );
		$html = wp_nonce_field( "wps-meta-delete-mupload_{$field['id']}", "nonce-delete-mupload_".$field['id'], false, false );
		...
		if( is_array( $meta ) && isset( $meta['src'] ) && $meta['src'] != '' ) {
			...
			$html .= "<input class='wps-meta-delete_image_button' type='button' rel='".$field['id']."' value='" . __( 'Delete Image', 'wpwfp' ) . "' />";
		} else {
			...
			$html .= "<input class='wps-meta-upload_image_button' type='button' rel='".$field['id']."' value='" . __( 'Upload Image', 'wpwfp' ) . "' />";
		}
	...
	public function show_field_color( $field, $meta ) {
		...
		if( wp_style_is( 'wp-color-picker', 'registered' ) ) { //iris color picker since 3.5
			echo "<input class='wps-meta-color-iris".( isset( $field['class'] )? " {$field['class']}": "")."' type='text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='8' />";  
		} else {
			echo "<input class='wps-meta-color".( isset( $field['class'] )? " {$field['class']}": "")."' type='text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='8' />";
			echo "<input type='button' class='wps-meta-color-select button' rel='{$field['id']}' value='" . __( 'Select a color' ,'wpwfp') . "'/>";
			echo "<div style='display:none' class='wps-meta-color-picker' rel='{$field['id']}'></div>";
		}
	...
	public function show_field_checkbox_list( $field, $meta ) {
		...
		foreach ($field['options'] as $key => $value) {
			$html[] = "<input type='checkbox' class='wps-meta-checkbox_list' name='{$field['id']}[]' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) . " /> {$value}";
		}
	...
	public function show_field_date( $field, $meta ) {
		...
		echo "<input type='text' class='wps-meta-date' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='{$meta}' size='30' />";
		$this->show_field_end( $field, $meta );
	}
	...
	public function show_field_datetime( $field, $meta ) {
		...
		echo "<input type='text' class='wps-meta-datetime' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='{$meta}' size='30' />";
		$this->show_field_end( $field, $meta );
	}
	...
	public function show_field_time( $field, $meta ) {
		...
		echo "<input type='text' class='wps-meta-time' name='{$field['id']}' id='{$field['id']}' data-ampm='{$ampm}' rel='{$field['format']}' value='{$meta}' size='30' />";
		$this->show_field_end( $field, $meta );
	}
	...
	*/
	...
	public function save( $post_id ) {
		...
		|| ( ! in_array( $post_type, $pages ) )              // Check if current post type is supported.
	 	|| ( ! check_admin_referer( WPW_FP_BASENAME, 'at_wpw_fp_meta_box_nonce') )      // Check nonce - Security
		...
		foreach ( $this->_fields as $field ) {	
			...
			if(isset($field['validate_func'])) {
				if ( class_exists( 'Wpw_Fp_Meta_Box_Validate' ) && method_exists( 'Wpw_Fp_Meta_Box_Validate', $field['validate_func'] ) ) {
					$wps_meta_validate = new Wpw_Fp_Meta_Box_Validate();
					$new = call_user_func( array( $wps_meta_validate, $field['validate_func'] ), $new );
				}
			}
	...
	*/
	...
	public function addWysiwyg( $id, $args ,$repeater=false ) {
		...
		$new_field = array( 'type' => 'wysiwyg', 'id'=> $id, 'std' => '', 'desc' => '', 'style' =>'width: 300px; height: 200px', 'name' => 'WYSIWYG Editor Field' );
		$new_field = array_merge( $new_field, $args );
	...
	*/
	...
	public function show_tab_begin() {
		...
		echo '<div class="wps-metabox-tabs-div">
