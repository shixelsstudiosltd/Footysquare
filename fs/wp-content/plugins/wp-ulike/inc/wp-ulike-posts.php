<?php		
	...
	function wp_ulike($arg) {
		...
		//update_field('likes', $counter, $post_ID);
	...
	function wp_ulike_posts_loggedby_username_method($post_ID,$return_userID,$type){
		...
		//pass likes count for custom post
		update_field('likes', $get_like, $post_ID);
		...
		if($type == 'post'){
			if($user_status == 0 && !isset($_COOKIE['liked-'.$post_ID])){
				...
				if (wp_ulike_get_setting( 'wp_ulike_general', 'button_type') == 'image') {
					if(!is_user_logged_in())
						$counter = '<a href="#login-model" class="image fancybox"></a><span class="count-box '.$hide.'">'.$liked.'</span>';
					else
					{
						$counter = '<a onclick="likeThis('.$post_ID.', 1, '.$num.');" class="image"></a><span class="count-box '.$hide.'">'.$liked.'</span>';
					}	
				}
				else {
					if(!is_user_logged_in())
						$counter = '<a href="#login-model" class="fancybox text">'.wp_ulike_get_setting( 'wp_ulike_general', 'button_text').'</a><span class="count-box">'.$liked.'</span>';
					else
				...
				if(wp_ulike_get_user_post_status($post_ID,$return_userID) == "like"){
					...
					if(wp_ulike_get_setting( 'wp_ulike_general', 'return_initial_after_unlike') == 1){
						...
						if(!is_user_logged_in())
							$counter = '<a href="#login-model"class="image fancybox"></a><span class="count-box">'.$liked.'</span>';		
						else
							$counter = '<a onclick="likeThis('.$post_ID.', 1, 0);" class="image"></a><span class="count-box">'.$liked.'</span>';		
						}
						else {
						if(!is_user_logged_in())
							$counter = '<a href="#login-model" class="text fancybox">'.wp_ulike_get_setting( 'wp_ulike_general', 'button_text').'</a><span class="count-box">'.$liked.'</span>';
						else
							$counter = '<a onclick="likeThis('.$post_ID.', 1, 0);" class="text">'.wp_ulike_get_setting( 'wp_ulike_general', 'button_text').'</a><span class="count-box">'.$liked.'</span>';
						}
					}
						...
						if(!is_user_logged_in())
							$counter = '<a href="#login-model" class="text fancybox">'.wp_ulike_get_setting( 'wp_ulike_general', 'text_after_unlike').'</a><span class="count-box">'.$liked.'</span>';
						else
							$counter = '<a onclick="likeThis('.$post_ID.', 1, 0);" class="text">'.wp_ulike_get_setting( 'wp_ulike_general', 'text_after_unlike').'</a><span class="count-box">'.$liked.'</span>';
					}
				}
