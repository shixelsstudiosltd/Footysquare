<?php
/**
 * Filter to parse post message and return YouTube as attachment
 **/
add_filter('wps_forum_item_filter', 'wps_parse_forum_youtube_links',10,4);
add_filter('wps_forum_item_comment_filter', 'wps_parse_forum_youtube_links',10,4);
function wps_parse_forum_youtube_links($item_html, $atts, $post, $content) {
	...
	$ids = get_forum_youtube_videos($content);
	if (!empty($ids)) {
		foreach ($ids as $id) {
...
function get_forum_youtube_videos($string) {
