 ...
 */
 ...
 if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == "frontend_ac_deactive") {
		...
		$login_url_out = site_url( '?page_id=2034', 'login' );
		$login_url_out = add_query_arg( 'disabled', '1', $login_url_out );
		wp_redirect( $login_url_out );
		exit;
		
		//include_once(ABSPATH . 'wp-includes/pluggable.php');
		//$login_url= 'http://localhost/fs/?page_id=2034';
...
final class ja_disable_users {
	...
	?>
		<form method="post" name="front_end_account_deactive" id="front_end_account_deactive" action="">
			<input type="hidden" name="ja_disable_user" id="ja_disable_user" value="1" />
			<input type="hidden" name="action" value="frontend_ac_deactive" />
			<input type="submit" value="deactivate" class="deactivate-account-btn"/>
		</form>
	<?php
	}
	...
	public function user_login( $user_login, $user ) {
		...
		//enable account if user get back
		update_user_meta( $user->ID, 'ja_disable_user', 0 );
		
		//$login_url_in = site_url( '?page_id=2037','http' );
		//$login_url_in = add_query_arg( 'u_return', '1', $login_url_in );
		//wp_redirect( $login_url_in );
