...
function wps_alerts_activity_list($atts) {
	...
	if ( is_user_logged_in() ):
		...
		if ($list):
			$html .= '<div style="max-width:100%" class="user-activity-sec user-latest-activity">';
				foreach ($list as $alert):
					//$html .= '<div data-url="'.$alert['url'].'" class="'.$alert['class'].'" value="'.$alert['ID'].'">';
					$html .= '<div class="'.$alert['class'].' col-lg-6">';
					$html .= '<p><i class="fa fa-clock-o"></i>'.sprintf($date_format, human_time_diff(strtotime($alert['post_date']), current_time('timestamp', 0)), WPS2_TEXT_DOMAIN).'</p>';
					$html .= '<div class="excerpt-value">'.$alert['excerpt'].'</div>';
					$html .= '</div>';
				endforeach;
			$html .= '</div>';
			$html .= '<div class="col-lg-12 see-all">SEE ALL</div>';
		else:
			$html .= $no_activity_text;
		endif;
...
add_shortcode(WPS_PREFIX.'-alerts-activity-list', 'wps_alerts_activity_list');
