...
class Wpw_Fp_User_Logs_List_Terms extends WP_List_Table {
    ...
    /**
	 * Displaying Followed Term User Logs
	 *
	 ...
	 * @since 1.1.0
	 */	
	function display_follow_term_user_logs() {
		...
		$data = $this->model->wpw_fp_get_follow_term_user_logs_data( $args );
    ...
    function column_mail_subject($item){
    	...
    	//Build row action
    	$actions['delete'] = sprintf('<a class="wpw-fp-logs-delete wpw-fp-delete" href="?page=%s&action=%s&userlog=%s&termid=%s&taxonomy=%s&logid=%s'.$pagestr.'">'.__('Delete', 'wpwfp').'</a>','wpw-fp-term','delete',$item['ID'],$_GET['termid'],$_GET['taxonomy'],$_GET['logid'], $pagenumber );
    ...
    function prepare_items() {
         ...
         */
		$data = $this->display_follow_term_user_logs();
...
$FollowedUserLogsTermListTable = new Wpw_Fp_User_Logs_List_Terms();
	
//Fetch, prepare, sort, and filter our data...
$FollowedUserLogsTermListTable->prepare_items();
...
<div class="wrap">
    <?php 
    	...
    	$email 		= get_post_meta( $_GET['logid'], $prefix.'term_user_email', true );
    	if( strlen( $title ) > 50 ) {
			$title = substr( $title, 0, 50 );
			$title = $title.'...';
		}
		//back url to go back on the page
		$backurl = add_query_arg( array( 'page' => 'wpw-fp-term', 'termid' => $_GET['termid'], 'taxonomy' => $_GET['taxonomy'] ), admin_url( 'admin.php' ) );
    ?>
    ...
    <?php 
    	...
    	$FollowedUserLogsTermListTable->views();
	...
	<div class="wpw_fp_view"><strong><?php _e( 'Term Title :', 'wpwfp' ); ?></strong></div>
    ...
    <form id="product-filter" method="get" class="wpw-fp-form">
		...
		<!-- Search Title -->
        <?php $FollowedUserLogsTermListTable->search_box( __( 'Search', 'wpwfp' ), 'wpwfp' ); ?>
        
        <!-- Now we can render the completed list table -->
        <?php $FollowedUserLogsTermListTable->display(); ?>
