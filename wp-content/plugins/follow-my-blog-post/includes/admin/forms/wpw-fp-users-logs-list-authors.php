...
/**
 * Followed Author User Logs List
 *
...
class Wpw_Fp_User_Logs_List_Authors extends WP_List_Table {
	...
	function display_follow_author_user_logs() {
		...
		$data = $this->model->wpw_fp_get_follow_author_user_logs_data( $args );
    ...
    function column_mail_subject($item){
    	...
    	//Build row action
    	$actions['delete'] = sprintf('<a class="wpw-fp-logs-delete wpw-fp-delete" href="?page=%s&action=%s&userlog=%s&authorid=%s&logid=%s'.$pagestr.'">'.__('Delete', 'wpwfp').'</a>','wpw-fp-author','delete',$item['ID'],$_GET['authorid'],$_GET['logid'], $pagenumber );
...
$FollowedUserLogsAuthorListTable = new Wpw_Fp_User_Logs_List_Authors();
...
<div class="wrap">
    <?php 
    	...
    	$email 		= get_post_meta( $_GET['logid'], $prefix.'author_user_email', true );
    	if( strlen( $title ) > 50 ) {
			$title = substr( $title, 0, 50 );
			$title = $title.'...';
		}
		//back url to go back on the page
		$backurl = add_query_arg( array( 'page' => 'wpw-fp-author', 'authorid' => $_GET['authorid']), admin_url( 'admin.php' ) );
    ?>
    ...
    <?php 
    	...
    	$FollowedUserLogsAuthorListTable->views();
	...
	<div class="wpw_fp_view"><strong><?php _e( 'Author Name :', 'wpwfp' ); ?></strong></div>
    ...
    <form id="product-filter" method="get" class="wpw-fp-form">
		...
		<!-- Search Title -->
        <?php $FollowedUserLogsAuthorListTable->search_box( __( 'Search', 'wpwfp' ), 'wpwfp' ); ?>
        
        <!-- Now we can render the completed list table -->
        <?php $FollowedUserLogsAuthorListTable->display(); ?>
