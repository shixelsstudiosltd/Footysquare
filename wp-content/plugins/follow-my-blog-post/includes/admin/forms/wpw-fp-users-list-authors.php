...
/**
 * Followed author Users List
 *
 * The html markup for the followed authors Users list
 * 
...
class Wpw_Fp_Users_List_Authors extends WP_List_Table {
    ...
    /**
	 * Displaying Followed author Users
	 *
	 * Does prepare the data for displaying followed author users in the table.
	 * 
	 * @package Follow My Blog Post
	 * @since 1.4
	 */	
	function display_follow_author_users() {
		...
		foreach ($data as $key => $value){
			...
			$permalink = add_query_arg( array( 'page' => 'wpw-fp-author', 'authorid' => $_GET['authorid'], 'logid' => $value['ID'] ), admin_url( 'admin.php' ) );
		...
		switch( $column_name ) {
			...
			case 'subscribed' 	:
				...
				if( $item[ $column_name ] == '1' ) {
					$status_url = add_query_arg( array( 'page' => 'wpw-fp-author', 'authorid' => $_GET['authorid'],'wpw_fp_status' => 'subscribe' ), $status_url );
					$status_link = '<a href="' . $status_url . '" >' . __( 'Yes', 'wpwfp' ) . '</a>';
				} else {
					$status_url = add_query_arg( array( 'page' => 'wpw-fp-author', 'authorid' => $_GET['authorid'], 'wpw_fp_status' => 'unsubscribe' ), $status_url );
    ...
    function column_user_email($item){
    	...
    	if( $item['subscribed'] == '1' ) {
    		$actions['unsubscribe'] = sprintf('<a href="?page=%s&action=%s&user[]=%s&authorid=%s'.$pagestr.'">'.__('Unsubscribe', 'wpwfp').'</a>','wpw-fp-author','unsubscribe',$item['ID'],$_GET['authorid'], $pagenumber);
    	} else {
    		$actions['subscribe'] = sprintf('<a href="?page=%s&action=%s&user[]=%s&authorid=%s'.$pagestr.'">'.__('Subscribe', 'wpwfp').'</a>','wpw-fp-author','subscribe',$item['ID'],$_GET['authorid'], $pagenumber);
    	}
    	
    	$actions['delete'] = sprintf('<a class="wpw-fp-users-delete wpw-fp-delete" href="?page=%s&action=%s&user=%s&authorid=%s'.$pagestr.'">'.__('Delete', 'wpwfp').'</a>','wpw-fp-author','delete',$item['ID'],$_GET['authorid'], $pagenumber );
    	...
    	if( $which == 'top' ) {
    		...
    		$html .= '<div class="alignleft actions">';
				...
				foreach ( $all_status as $key => $status ) {
					...
					$status_count = $this->model->wpw_fp_get_follow_author_users_data( $args );
    ...
    function prepare_items() {
         ...
         */
		$data = $this->display_follow_author_users();
...
$FollowedUsersAuthorListTable = new Wpw_Fp_Users_List_Authors();
	
//Fetch, prepare, sort, and filter our data...
$FollowedUsersAuthorListTable->prepare_items();
...
<div class="wrap">
    <?php 
    	...
    	$title 		= isset( $author_data->display_name ) ? $author_data->display_name : '';
    	if( strlen( $title ) > 50 ) {
			$title = substr( $title, 0, 50 );
			$title = $title.'...';
		}
		//back url to go back on the page
		$backurl = add_query_arg( array( 'page' => 'wpw-fp-author' ), admin_url( 'admin.php' ) );
    ?>
    ...
    <?php 
    	...
    	$FollowedUsersAuthorListTable->views(); 
    ...
    <form id="product-filter" method="get" class="wpw-fp-form">
        ...
        <input type="hidden" name="authorid" value="<?php echo  $_GET['authorid']; ?>" />
        
        <!-- Search Title -->
        <?php $FollowedUsersAuthorListTable->search_box( __( 'Search', 'wpwfp' ), 'wpwfp' ); ?>
        
        <!-- Now we can render the completed list table -->
        <?php $FollowedUsersAuthorListTable->display(); ?>
