					...
					$.post(opts.phpPath,{
						...
						function(data) {
							if(!data.error)
							{
								/** ONLY FOR THE DEMO, YOU CAN REMOVE THIS CODE **/
									$('#rating_loading').hide();
									$('#rating_loading2').hide();
									$('.jRatingAverage').css("width","0");
									var strs = data.server.split(" ");
									//alert( strs[0] );
									$('.jRatingColor').css("width",strs[0]*10+"%");
									$('.jRatingColor2').css("width",strs[0]*10+"%");
									$('#jRatingColor2').show();
									$('.basic').show();
									$('#rating_saved').html(data.server).show();
									$('#rating_saved2').html(data.server).show();
								/** END ONLY FOR THE DEMO **/
